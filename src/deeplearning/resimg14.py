...
class ResNet(nn.Module):
    def __init__(self, ResBlock, num_classes=10):
        ...
        self.layer3 = self.make_layer(ResBlock, 128, 1, stride=2)
        self.layer4 = self.make_layer(ResBlock, 256, 1, stride=2)
...
def ResImgNet14(num_classes):
    return ResNet(ResidualBlock, num_classes=num_classes)


def info(num_classes=10):
    net = ResImgNet14(num_classes=num_classes).to(device)
    summary(net, (1, 3, 56, 56))
...
def pred(model, testloader):
    # net = ResImgNet14().to(device)
    # net.load_state_dict(model)
    ann.pred(model, testloader, show_confusion=True)
